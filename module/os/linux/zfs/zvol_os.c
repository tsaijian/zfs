...
zvol_strategy_epilogue(void *arg)
{
	...
	unsigned long start_time = zss->start_jif;
	int err;
	boolean_t reader;

	reader = !!(dc->dc_flags & DMU_CTX_FLAG_READ);
	err = dc->dc_err;
	blk_generic_end_io_acct(q, disk, reader ? READ : WRITE, bio,
	    start_time);
	BIO_END_IO(bio, -err);
	ASSERT3S(atomic_read(&zv->zv_suspend_ref), >, 0);
...
zvol_strategy(void *arg)
{
	...
	blk_generic_start_io_acct(q, disk, rw, bio);
