...
static int
zfs_mount_set_schg(const char *mountpoint)
{
	int r;
#ifdef __linux__
	int fd;
...
out:
	...
	if (r < 0)
		return (r);
	s.st_flags |= (SF_IMMUTABLE);
	r = chflags(mountpoint, s.st_flags);
#endif
	return (r);
}
    ...
    const char *mountpoint)
{
	...
	if (set_schg) {
		if (zfs_mount_set_schg(mountpoint) < 0) {
			return (zfs_error_fmt(hdl, EZFS_MOUNTFAILED,
