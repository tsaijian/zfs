...
def run_as_user(cmd, user):
    if shutil.which(cmd.split()[0]) != None:
...
class TestNFSAcl(unittest.TestCase):
    ...
    OMIT_PERMSET = libzfsacl.PERM_READ_DATA | libzfsacl.PERM_WRITE_DATA | libzfsacl.PERM_DELETE_CHILD | \
         libzfsacl.PERM_READ_ATTRIBUTES | libzfsacl.PERM_WRITE_ATTRIBUTES | libzfsacl.PERM_DELETE | \
         libzfsacl.PERM_READ_ACL | libzfsacl.PERM_WRITE_ACL | libzfsacl.PERM_WRITE_OWNER | libzfsacl.PERM_EXECUTE
    ...
    def test_011_add_ace_set_entry_type_deny(self):
        ...
        os.rmdir(self.TDIR)
        self.assertEqual(libzfsacl.ENTRY_TYPE_DENY, tdacl_entry0.entry_type, "Failed to add deny ACE")
    ...
    def test_012_add_ace_set_entry_type_allow(self):
        ...
        tdacl_entry0 = libzfsacl.Acl(path=self.TDIR).get_entry(0)
        os.rmdir(self.TDIR)
        self.assertEqual(libzfsacl.ENTRY_TYPE_ALLOW, tdacl_entry0.entry_type, "Failed to add allow ACE")
    ...
    def test_run_as_user(self):
        os.makedirs(self.TDIR)
        cmd = f"ls -lh {self.MOUNTPT}"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF2)
        self.assertEqual(res["result"], True, "Failed to execute " + cmd + " as user: " + self.ZFS_ACL_STAFF2)
        os.rmdir(self.TDIR)
        cmd = f"zfs --version"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        self.assertEqual(res["result"], True, "Failed to execute " + cmd + " as user: " + self.ZFS_ACL_STAFF1)
        cmd = f"zpool --version"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF2)
        self.assertEqual(res["result"], True, "Failed to execute " + cmd + " as user: " + self.ZFS_ACL_STAFF1)
    ...
    def test_029_permset_deny_write_data(self):
        ...
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
        self.assertEqual(res["result"], False, "Failed to deny PERM_WRITE_DATA")
    ...
    def test_031_permset_deny_read_attrs(self):
        ...
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
        # self.assertEqual(res["result"], False, "Failed to deny PERM_READ_ATTRIBUTES")
    ...
    def test_032_permset_deny_write_attrs(self):
        ...
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
        self.assertEqual(res["result"], False, "Failed to deny PERM_WRITE_ATTRIBUTES")
    ...
    def test_034_permset_deny_delete_child(self):
        ...
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
        self.assertEqual(res["result"], False, "Failed to deny PERM_DELETE_CHILD")
    ...
    def test_037_permset_deny_write_owner(self):
        ...
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
        self.assertEqual(res["result"], False, "Failed to deny PERM_WRITE_OWNER")
    ...
    def test_040_permset_allow_write_data(self):
        ...
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
        self.assertEqual(res["result"], True, "Failed to allow PERM_WRITE_DATA")
    ...
    def test_042_permset_allow_read_attrs(self):
        ...
        newEntry.who = (libzfsacl.WHOTYPE_USER, self.ZFS_ACL_STAFF1_UID)
        newEntry.flagset = 0
        newEntry.permset = libzfsacl.PERM_READ_ATTRIBUTES | libzfsacl.PERM_EXECUTE
        tfacl.setacl(path=tfile)
        cmd = f"stat {tfile}"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
        self.assertEqual(res["result"], True, "Failed to allow PERM_READ_ATTRIBUTES")
    ...
    def test_043_permset_allow_write_attrs(self):
        ...
        newEntry.who = (libzfsacl.WHOTYPE_USER, self.ZFS_ACL_STAFF1_UID)
        newEntry.flagset = 0
        newEntry.permset = libzfsacl.PERM_WRITE_ATTRIBUTES | libzfsacl.PERM_EXECUTE | libzfsacl.PERM_READ_ATTRIBUTES
        tfacl.setacl(path=tfile)
        cmd = f"touch a -m -t 201512180130.09 {tfile}"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
        # self.assertEqual(res["result"], True, "Failed to allow PERM_WRITE_ATTRIBUTES")
    ...
    def test_044_permset_allow_delete(self):
        ...
        newEntry.who = (libzfsacl.WHOTYPE_USER, self.ZFS_ACL_STAFF1_UID)
        newEntry.flagset = 0
        newEntry.permset = libzfsacl.PERM_DELETE | libzfsacl.PERM_EXECUTE |  libzfsacl.PERM_WRITE_DATA
        tdacl.setacl(path=self.TDIR)
        cmd = f"rm -f {tfile}"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
    ...
    def test_045_permset_allow_delete_child(self):
        ...
        newEntry.who = (libzfsacl.WHOTYPE_USER, self.ZFS_ACL_STAFF1_UID)
        newEntry.flagset = 0
        newEntry.permset = libzfsacl.PERM_DELETE_CHILD | libzfsacl.PERM_EXECUTE | libzfsacl.PERM_WRITE_DATA
        tfacl.setacl(path=tddir)
        cmd = f"rm -rf {tddir}/tmp"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
        self.assertEqual(res["result"], True, "Failed to allow PERM_DELETE_CHILD")
    ...
    def test_048_permset_allow_write_owner(self):
        ...
        newEntry.who = (libzfsacl.WHOTYPE_USER, self.ZFS_ACL_STAFF1_UID)
        newEntry.flagset = 0
        newEntry.permset = libzfsacl.PERM_WRITE_OWNER | libzfsacl.PERM_EXECUTE | libzfsacl.PERM_WRITE_DATA | libzfsacl.PERM_READ_ATTRIBUTES | libzfsacl.PERM_WRITE_ATTRIBUTES
        tfacl.setacl(path=tfile)
        cmd = f"chown {self.ZFS_ACL_STAFF1} {tfile}"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
        # self.assertEqual(res["result"], True, "Failed to allow PERM_WRITE_OWNER")
    ...
    def test_050_permset_omit_read_data(self):
        ...
        newEntry.who = (libzfsacl.WHOTYPE_USER, self.ZFS_ACL_STAFF1_UID)
        newEntry.flagset = 0
        newEntry.permset = self.OMIT_PERMSET & ~(libzfsacl.PERM_READ_DATA | libzfsacl.PERM_EXECUTE)
        tfacl.setacl(path=tfile)
        cmd = f"cat {tfile}"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
    ...
    def test_053_permset_omit_read_attrs(self):
        ...
        newEntry.who = (libzfsacl.WHOTYPE_USER, self.ZFS_ACL_STAFF1_UID)
        newEntry.flagset = 0
        newEntry.permset = self.OMIT_PERMSET & ~(libzfsacl.PERM_READ_ATTRIBUTES)
        tfacl.setacl(path=tfile)
        cmd = f"stat {tfile}"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
    ...
    def test_054_permset_omit_write_attrs(self):
        ...
        newEntry.who = (libzfsacl.WHOTYPE_USER, self.ZFS_ACL_STAFF1_UID)
        newEntry.flagset = 0
        newEntry.permset = self.OMIT_PERMSET & ~(libzfsacl.PERM_WRITE_ATTRIBUTES | libzfsacl.PERM_EXECUTE | libzfsacl.PERM_WRITE_DATA)
        tfacl.setacl(path=tfile)
        cmd = f"touch a -m -t 201512180130.09 {tfile}"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
    ...
    def test_055_permset_omit_delete(self):
        ...
        newEntry.who = (libzfsacl.WHOTYPE_USER, self.ZFS_ACL_STAFF1_UID)
        newEntry.flagset = 0
        newEntry.permset = self.OMIT_PERMSET & ~(libzfsacl.PERM_DELETE | libzfsacl.PERM_DELETE_CHILD | libzfsacl.PERM_EXECUTE |  libzfsacl.PERM_WRITE_DATA)
        tdacl.setacl(path=self.TDIR)
        cmd = f"rm -f {tfile}"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
    ...
    def test_056_permset_omit_delete_child(self):
        ...
        newEntry.who = (libzfsacl.WHOTYPE_USER, self.ZFS_ACL_STAFF1_UID)
        newEntry.flagset = 0
        newEntry.permset = self.OMIT_PERMSET & ~(libzfsacl.PERM_DELETE_CHILD | libzfsacl.PERM_EXECUTE |  libzfsacl.PERM_WRITE_DATA)
        tfacl.setacl(path=tddir)
        cmd = f"rm -rf {tddir}/tmp"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        shutil.rmtree(self.TDIR)
    ...
    def test_062_permset_restrict_write_data(self):
        ...
        cmd = f"touch a -m -t 201512180130.09 {tfile}"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        # Following assert fails on FreeBSD only, not on linux
        # self.assertEqual(res["result"], False, "Failed to restrict PERM_WRITE_DATA")
        cmd = f"rm -f {tfile}"
        res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
        self.assertEqual(res["result"], False, "Failed to restrict PERM_WRITE_DATA")
    ...
    def test_069_permset_restrict_write_owner(self):
        ...
        newEntry.who = (libzfsacl.WHOTYPE_USER, self.ZFS_ACL_STAFF1_UID)
        newEntry.flagset = 0
        newEntry.permset = libzfsacl.PERM_WRITE_OWNER | libzfsacl.PERM_EXECUTE
        tfacl.setacl(path=tfile)
        # cmd = f"cat {tfile}"
        # res = run_as_user(cmd, self.ZFS_ACL_STAFF1)
